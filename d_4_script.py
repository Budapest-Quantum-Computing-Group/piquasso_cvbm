...
FILENAME = f"{folder_name}/4_mode_{now.day}_{now.hour}_{now.minute}_{shots}_{seed_sequence}.json"
...
def get_samples(simulator, weights, shots):
    with pq.Program() as program:
        ...
        pq.Q(1) | pq.Displacement(-1.0)

        pq.Q(2) | pq.Displacement(1.0, phi=np.pi/2)
        pq.Q(3) | pq.Displacement(1.0, phi=-np.pi/2)

        pq.Q(1, 2) | pq.CrossKerr(xi=np.pi / 3)

        pq.Q(0, 1) | pq.Beamsplitter(weights[0])
        pq.Q(2, 3) | pq.Beamsplitter(weights[1])
...
if __name__ == "__main__":
    ...
    weights_to_learn = np.array([np.pi / 4, -np.pi / 4])
